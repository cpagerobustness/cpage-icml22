...
def Train(model, writer, t, loader, eps_scheduler, norm, train, opt, bound_type, method='robust'):
	...
	for i, (data, labels) in enumerate(loader):
		...
		for j in range(data.size(0)):
			...
			#if (labels[j] >= 0 and labels[j] < 3): nlabel[j] = 0
			#elif (labels[j] >= 3 and labels[j] < 6): nlabel[j] = 1
			#else: nlabel[j] = 2
		start = time.time()
		...
		if batch_method != "natural":
			...
			meter.update('Certified_acc_ens', torch.sum((lb >= 0).all(dim=1)).item() / data.size(0) * 100., data.size(0))
			#for j in range(m):
...
def __main__():
	...
	for t in range(1, epochs + 1):
		...
		Train(model_pred, writer, t, train_data, eps_scheduler, norm, True, opt, "CROWN-IBP")
		epoch_time = time.time() - start_time
		timer += epoch_time
		print('Epoch time: {:.4f}, Total time: {:.4f}'.format(epoch_time, timer))
		print("Evaluating...")
		with torch.no_grad():
			Train(model_pred, writer, t, test_data, eps_scheduler, norm, False, None, "CROWN-IBP")
		for i in range(m):
			torch.save({'state_dict': model.state_dict(), 'epoch': t},
						"ngate_0.3/model%d.pt" % (i))
__main__()
