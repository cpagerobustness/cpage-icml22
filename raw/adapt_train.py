...
def Train(model, ens, gate, writer, t, loader, eps_scheduler, norm, train, opt, bound_type, method='robust'):
	num_class = 10
	...
	for i, (data, labels) in enumerate(loader):
		...
		c_label = -torch.ones_like(labels).type(torch.LongTensor)
		...
		for k in range(data.size(0)):
			...
			if (chosen != 1): 
				if (chosen == 0): c_label[k] = -2
				continue
			c_label[k] = idx

		regular_ce = 0

		weights = [1 + 15 * ((c_label == -1) | (c_label == 0)), 1 + 15 * ((c_label == -1) | (c_label == 1)), 1 + 15 * ((c_label == -1) | (c_label == 1))]

		for j in range(m): 
			output = model[j](x)
			regular_ce += torch.mean(CrossEntropyLoss(reduce=False)(output, labels) * weights[j])
		...
		if batch_method == "robust":
			...
			for j in range(m):
				...
				target[:, j] = (clb >= 0).all(dim=1, keepdim=True).cpu().detach().squeeze()
				# Pad zero at the beginning for each example, and use fake label "0" for all examples
				lb_padded = torch.cat((torch.zeros(size=(lb.size(0),1), dtype=lb.dtype, device=lb.device), lb), dim=1)
				fake_labels = torch.zeros(size=(lb.size(0),), dtype=torch.int64, device=lb.device)
				robust_ce += torch.mean(CrossEntropyLoss(reduce=False)(-lb_padded, fake_labels) * weights[j])
...
def __main__():
	...
	for t in range(1, epochs + 1):
		...
		Train(models, ens, gate_pred, writer, t, train_data, eps_scheduler, norm, True, opt, "CROWN-IBP")
		epoch_time = time.time() - start_time
		timer += epoch_time
		print('Epoch time: {:.4f}, Total time: {:.4f}'.format(epoch_time, timer))
		print("Evaluating...")
		with torch.no_grad():
			Train(models, ens, gate_pred, writer, t, test_data, eps_scheduler, norm, False, None, "CROWN-IBP")
		for i in range(m):
			torch.save({'state_dict': core[i].state_dict(), 'epoch': t},
						"adapt_cibp_0.3_15/model%d.pt" % (i))
__main__()
